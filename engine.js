...
/* 2. Price-action trend via EMA stack + slope.
   Long-EMA period adapts to available history so the macro filter still works
   on a short/resampled series (e.g. early HTF bars) instead of silently
   disabling itself when a fixed EMA200 would be null. */
function trendEMA(c) {
  const cl = c.map(x => x.c);
  const i = c.length - 1;
  // long-EMA period: 200 when we have the history, otherwise the largest period
  // that still leaves a clear gap above the 50-EMA (min 80) so the macro filter
  // stays meaningful on short/resampled series instead of collapsing to ==50.
  const longP = (i + 1) >= 200 ? 200 : Math.max(80, Math.min(200, Math.floor((i + 1) * 0.85)));
  const e20 = M.ema(cl, 20), e50 = M.ema(cl, 50), eLong = M.ema(cl, longP);
  let score = 0, parts = [];
  if (e20[i] && e50[i]) { const up = e20[i] > e50[i]; score += up ? 0.4 : -0.4; parts.push(up ? '20>50 EMA' : '20<50 EMA'); }
  if (e50[i] && eLong[i] && longP > 50) { const up = e50[i] > eLong[i]; score += up ? 0.4 : -0.4; parts.push(up ? `50>${longP} EMA (bull)` : `50<${longP} EMA (bear)`); }
  if (e20[i] && e20[i - 5]) { const up = e20[i] > e20[i - 5]; score += up ? 0.2 : -0.2; parts.push(up ? 'rising slope' : 'falling slope'); }
  return { score: Math.max(-1, Math.min(1, score)), label: 'Trend (EMA Stack)', detail: parts.join(' · '), ema: { e20, e50, e200: eLong } };
}
...
function backtest(daily, usdtAligned, params, opts) {
  ...
  for (let i = warmup; i < daily.length - 1; i++) {
    ...
    for (let j = i + 1; j <= end && !closed; j++) {
      ...
      const nextTpHit = filled < 3 && (isLong ? bar.h >= tps[filled] : bar.l <= tps[filled]);
      // Same-bar ambiguity: if BOTH the stop and the next TP are inside this
      // bar's range, assume the level nearer to the bar's OPEN is reached first
      // (a fairer proxy than always assuming the stop). Only the stop-first case
      // forces a full close; otherwise TPs process then a later bar may stop out.
      let stopFirst = stopHit;
      if (stopHit && nextTpHit) {
        const dStop = Math.abs(bar.o - curStop);
        const dTp = Math.abs(bar.o - tps[filled]);
        stopFirst = dStop <= dTp;
      }
      // 1) stop (full close) when it wins the bar
      if (stopFirst) {
        const px = curStop;
      ...
      // 3) if the stop was also in-range this bar but TP came first, honour it now
      if (!closed && stopHit && (isLong ? bar.l <= curStop : bar.h >= curStop)) {
        const px = curStop;
        const pnl = isLong ? remaining * (px - entry) : remaining * (entry - px);
        grossPnl += pnl; exitFees += feeOf(remaining * px, 'taker');
        fills.push({ price: px, qty: remaining, reason: curStop === entry ? 'BE-stop' : 'stop' });
...
function score(s) {
  ...
  if (!s || s.n < 8) return -Infinity;
  const pf = isFinite(s.profitFactor) ? s.profitFactor : 3;
  return (s.returnPct / 100) * Math.sqrt(s.n) * Math.min(pf, 3) / (1 + s.maxDDpct / 100 * 1.5);
}
