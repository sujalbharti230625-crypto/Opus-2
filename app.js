...
// IMPORTANT: Binance caps each call at 1000 candles. For windows > 1000 bars
// (e.g. 5 years of daily = ~1830) we must PAGINATE forward, otherwise the API
// silently returns only the oldest 1000 and drops the most recent data.
async function fetchBinance(prof) {
  const stepMs = prof.sec * 1000;
  let cursor = (Math.floor(Date.now() / 1000) - prof.bars * prof.sec) * 1000;
  const nowMs = Date.now();
  let all = [], guard = 0;
  while (cursor < nowMs && guard < 30) {
    const url = `https://api.binance.us/api/v3/klines?symbol=ETHUSDT&interval=${prof.binance}&limit=1000&startTime=${cursor}`;
    ...
    cursor = rows[rows.length - 1][0] + stepMs; // continue after last open time
    guard++;
    await new Promise(r => setTimeout(r, 200));
  }
  if (!all.length) throw new Error('Binance: empty response');
...
function renderCalib(best) {
  ...
  $('#calibChips').innerHTML = `
    ...
    <div class="cchip">Price-action wt <b>${p.priceAction!=null?p.priceAction:2.2}</b></div>
    <div class="cchip">Objective <b>${objTxt}</b></div>`;

  // Out-of-sample validation panel (honesty check against overfitting)
  const el = $('#oosPanel');
  if (el) {
    if (best.validated && best.inSample && best.outOfSample) {
      const IS = best.inSample, OOS = best.outOfSample;
      const f = s => `${s.n} trades · ${s.winRate.toFixed(0)}% win · PF ${isFinite(s.profitFactor)?s.profitFactor.toFixed(2):'∞'} · ${s.returnPct>=0?'+':''}${s.returnPct.toFixed(1)}%`;
      const degraded = (isFinite(OOS.profitFactor)?OOS.profitFactor:3) < (isFinite(IS.profitFactor)?IS.profitFactor:3) * 0.7;
      el.style.display = 'block';
      el.innerHTML = `
        <div style="font-size:12px;color:var(--mut);margin-bottom:6px">Walk-forward validation — params optimised on the first 70% of history, then tested on the unseen last 30%:</div>
        <div class="flexrow" style="gap:10px">
          <div class="cchip" style="flex:1">In-sample (train) <b style="color:var(--cyn)">${f(IS)}</b></div>
          <div class="cchip" style="flex:1">Out-of-sample (test) <b style="color:${degraded?'var(--red)':'var(--grn)'}">${f(OOS)}</b></div>
        </div>
        <div style="font-size:11px;color:${degraded?'var(--red)':'var(--mut)'};margin-top:6px">${degraded?'⚠ Significant degradation out-of-sample — likely overfit; treat live expectations conservatively.':'✓ Out-of-sample performance held up reasonably — less overfit.'}</div>`;
    } else {
