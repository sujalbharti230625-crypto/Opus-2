...
(() => {
  ...
  // --- out-of-sample validation (overfitting guard) ---
  const big = randomWalk(900, 11);
  const calV = E.calibrate(big, null, null, { htf: 5, mtf: 2, warmup: 210 });
  ok(calV.best.validated === true, 'calibrate runs walk-forward validation on long data');
  ok(calV.best.inSample && calV.best.outOfSample, 'calibrate reports in-sample & out-of-sample stats');
  ok(typeof calV.best.splitIdx === 'number' && calV.best.splitIdx < big.length, 'validation split index is within range');
  // full reported stats should cover the whole series (>= train sample)
  ok(calV.best.stats.n >= 0, 'full-history stats present');
  // can be disabled
  const calNV = E.calibrate(big, null, null, { validate: false });
  ok(calNV.best.validated === false, 'validation can be disabled via opts.validate=false');
})();

section('Backtest 5-year scale & same-bar ambiguity');
(() => {
  // long 5yr-scale series must run without crashing and keep equity finite
  const long = randomWalk(1830, 21);
  const bt = E.backtest(long, null, {}, { capital: 10000, riskPct: 1, style: 'swing' });
  finiteNum(bt.stats.finalEquity, '5yr-scale finalEquity finite');
  finiteNum(bt.stats.maxDDpct, '5yr-scale maxDDpct finite');
  ok(bt.curve.length === bt.stats.n + 1, '5yr curve length = trades + seed');
  // fills always reconcile to position qty
  let mism = 0; bt.trades.forEach(t => { const fq = t.fills.reduce((a, f) => a + f.qty, 0); if (Math.abs(fq - t.qty) > 1e-6) mism++; });
  eq(mism, 0, 'every trade: partial fills reconcile to position size');

  // same-bar stop+TP: a giant bar engulfing both must resolve to exactly one full close
  const c = [];
  for (let i = 0; i < 260; i++) c.push({ t: 1700000000 + i * 86400, o: 100, h: 100.5, l: 99.5, c: 100, v: 1 });
  // craft a strong uptrend tail so a LONG triggers, then a huge engulfing bar
  let p = 100; for (let i = 0; i < 40; i++) { p *= 1.03; c.push({ t: 1700000000 + (260 + i) * 86400, o: p / 1.03, h: p * 1.01, l: p * 0.99, c: p, v: 1 }); }
...
(() => {
  // a clean uptrend of only 80 bars: fixed EMA200 would be null, adaptive should still vote
  let p = 1000; const up = [];
  for (let i = 0; i < 80; i++) { p *= 1.01; up.push({ t: 1700000000 + i * 86400, o: p / 1.01, h: p * 1.005, l: p * 0.997, c: p, v: 1 }); }
  const tr = E.trendEMA(up);
  finiteNum(tr.score, 'adaptive trendEMA score finite on 80 bars');
  ok(tr.score > 0, 'adaptive trendEMA detects uptrend without a full EMA200');
  ok(/EMA \(bull\)/.test(tr.detail), 'adaptive long-EMA contributes to the trend vote');
})();
